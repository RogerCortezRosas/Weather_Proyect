...
with DAG(dag_id='Weather_ETL',description='This dag is to extract , transform and load the date from the API of weather',
    ...
    check_new_data = SqlSensor(
        ...
        conn_id= 'mysql_conn',
        sql=" SELECT COUNT(id) FROM pressure WHERE id> (SELECT MAX(id) FROM pressure) -10",
        mode = 'poke',
        timeout = 300,
    ...
    Transform_pyspark = TransformPyspark(task_id = 'Transform_data_pyspark',tables=['pressure','weather_description','wind_direction'],depends_on_past=True,trigger_rule = TriggerRule.ALL_SUCCESS)
    ...
    Delete_Pysapark = Drop(task_id = 'Delete_Pyspark',tables=['pressure','weather_description','wind_direction'],depends_on_past=False)
